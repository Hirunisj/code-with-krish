...
export class ProductsService {
  ...
  async consumerOrderCreated() {
    await this.consumer.subscribe({ topic: 'hiruni.order.create' });
    ...
    await this.consumer.run({
      eachMessage: async ({ message }) => {
        ...
        const { customerId, customerName, items } = JSON.parse(messageValue);
        ...
        //release lock
        for (const item of items ){
          const lockKey = `hiruni.product:${item.productId};lock`;
          const lock = await this.redis.del(lockKey);
        }
        await this.producer.send({
          topic: 'hiruni.inventory.update',
